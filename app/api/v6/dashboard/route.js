    ...
    if (effectiveRunId) {
      stats = await db.execute(sql`
        SELECT
          ...
          (SELECT count(*) FROM coverage_assessments ca WHERE ca.map_run_id = ${effectiveRunId} AND COALESCE(ca.human_status, ca.status) = 'NOT_APPLICABLE') as not_applicable,
      ...
      stats = await db.execute(sql`
        SELECT
          ...
          (SELECT count(*) FROM coverage_assessments WHERE COALESCE(human_status, status) = 'NOT_APPLICABLE') as not_applicable,
