import { getRemediationItems } from '@/lib/remediation';
...
export async function GET(req, { params }) {
  try {
    ...
    const policyNumber = normalizeString(searchParams.get('policyNumber'));
    if (!policyNumber) {
      return Response.json({ error: 'Missing policyNumber' }, { status: 400 });
    }

    const items = await getRemediationItems(runId, policyNumber, getFilters(searchParams));
    return Response.json({ items });
  } catch (err) {
    console.error('Remediation items error:', err);
