...
// ── COVERAGE ASSESSMENT PROMPT (v7: facility context, N/A, 3-step decision procedure) ──
...
Review EVERY numbered provision from EVERY candidate policy above.
Do not stop after finding the first relevant provision — scan all provisions
before reaching your conclusion.

  COVERED = A provision fully addresses every element of the obligation:
    actor, action, condition, timeframe, documentation requirement.
    All elements must be present. Vague or general references do not count.

  PARTIAL = The topic is addressed but specific elements are missing.
    You MUST list exactly which elements are missing in gap_detail.

  GAP = No candidate policy meaningfully addresses this requirement.
    GAP means no candidate policy touches this subject at all. If a provision
    addresses the same topic but takes a different approach or appears to
    contradict the obligation, the status is PARTIAL or CONFLICTING — never GAP.

For COVERED, PARTIAL, or GAP: you must list in reviewed_provision_refs
every provision you considered relevant (minimum 2 unless fewer than 2
provisions exist across all candidates).
